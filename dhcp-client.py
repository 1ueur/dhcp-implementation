...
class DHCP_client(object):
    ...
    def discover_get():
        ...
        #
        XID = bytes([0x39, 0x03, 0xF3, 0x26]) # Same Transaction ID
        SECS = bytes([0x00, 0x00])
        FLAGS = bytes([0x80, 0x00]) # Flags (0x8000: Broadcast Flag)
        CIADDR = bytes([0x00, 0x00, 0x00, 0x00])
        YIADDR = bytes([0x00, 0x00, 0x00, 0x00])
        SIADDR = bytes([0x00, 0x00, 0x00, 0x00])
        GIADDR = bytes([0x00, 0x00, 0x00, 0x00])

        CHADDR1 = bytes([0xb6, 0xaa, 0x0C, 0x80]) # Client MAC Address
        CHADDR2 = bytes([0xaf, 0x21, 0x00, 0x00]) 
        CHADDR3 = bytes([0x00, 0x00, 0x00, 0x00]) 
        CHADDR4 = bytes([0x00, 0x00, 0x00, 0x00]) 
        CHADDR5 = bytes(192)
        Magiccookie = bytes([0x63, 0x82, 0x53, 0x63])
        DHCPOptions1 = bytes([53 , 1 , 1]) # DHCP Message Type (Discover)
        DHCPOptions2 = bytes([50 , 4 , 0xC0, 0xA8, 0x00, 0x09]) # Requested IP Address: 192.168.0.9
    ...
    def request_get():
        ...
        CHADDR5 = bytes(192)
        Magiccookie = bytes([0x63, 0x82, 0x53, 0x63])
        DHCPOptions1 = bytes([53 , 1 , 3]) # DHCP Message Type (Request)
        DHCPOptions2 = bytes([50 , 4 , 0xC0, 0xA8, 0x00, 0x09]) # Requested IP Address: 192.168.0.9
        DHCPOptions3 = bytes([54 , 4 , 0xC0, 0xA8, 0x00, 0x01]) # DHCP Server Identifier: 192.168.0.1
